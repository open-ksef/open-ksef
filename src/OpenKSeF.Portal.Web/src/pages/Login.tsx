import { useState, type FormEvent, type ReactElement } from 'react'
import { Navigate, useNavigate } from 'react-router-dom'

import { useAuth } from '@/auth/useAuth'

type AuthTab = 'login' | 'register'

export function LoginPage(): ReactElement {
  const { isAuthenticated, isLoading, loginWithCredentials, loginWithGoogle, register } = useAuth()
  const navigate = useNavigate()
  const [tab, setTab] = useState<AuthTab>('login')
  const [error, setError] = useState<string | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const [loginEmail, setLoginEmail] = useState('')
  const [loginPassword, setLoginPassword] = useState('')

  const [regEmail, setRegEmail] = useState('')
  const [regPassword, setRegPassword] = useState('')
  const [regConfirmPassword, setRegConfirmPassword] = useState('')
  const [regFirstName, setRegFirstName] = useState('')
  const [regLastName, setRegLastName] = useState('')

  if (isLoading) {
    return <main className="auth-page"><div className="auth-card">Sprawdzanie uwierzytelnienia…</div></main>
  }

  if (isAuthenticated) {
    return <Navigate to="/" replace />
  }

  async function handleLogin(e: FormEvent): Promise<void> {
    e.preventDefault()
    setError(null)
    setIsSubmitting(true)
    try {
      await loginWithCredentials(loginEmail, loginPassword)
      navigate('/', { replace: true })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Nieprawidłowy email lub hasło.')
    } finally {
      setIsSubmitting(false)
    }
  }

  async function handleRegister(e: FormEvent): Promise<void> {
    e.preventDefault()
    setError(null)

    if (regPassword !== regConfirmPassword) {
      setError('Hasła nie są identyczne.')
      return
    }

    if (regPassword.length < 8) {
      setError('Hasło musi mieć co najmniej 8 znaków.')
      return
    }

    setIsSubmitting(true)
    try {
      await register({
        email: regEmail,
        password: regPassword,
        firstName: regFirstName || undefined,
        lastName: regLastName || undefined,
      })
      navigate('/', { replace: true })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Rejestracja nie powiodła się.')
    } finally {
      setIsSubmitting(false)
    }
  }

  function switchTab(next: AuthTab): void {
    setTab(next)
    setError(null)
  }

  return (
    <main className="auth-page">
      <div className="auth-card">
        <div className="auth-logo" aria-hidden="true">⚡</div>
        <h1 className="auth-card__title">OpenKSeF</h1>
        <p className="auth-card__desc">
          {tab === 'login'
            ? 'Zaloguj się do portalu OpenKSeF'
            : 'Utwórz nowe konto'}
        </p>

        {error && (
          <div className="ui-form-error auth-card__error" role="alert">
            <span className="ui-form-error__icon" aria-hidden="true">!</span>
            {error}
          </div>
        )}

        {tab === 'login' ? (
          <>
            <form className="auth-card__form" onSubmit={(e) => void handleLogin(e)}>
              <div className="ui-form-group">
                <label htmlFor="login-email">Email</label>
                <input
                  id="login-email"
                  type="text"
                  autoComplete="username"
                  placeholder="jan@firma.pl"
                  value={loginEmail}
                  onChange={(e) => setLoginEmail(e.target.value)}
                  required
                  disabled={isSubmitting}
                  data-testid="login-email"
                />
              </div>
              <div className="ui-form-group">
                <label htmlFor="login-password">Hasło</label>
                <input
                  id="login-password"
                  type="password"
                  autoComplete="current-password"
                  placeholder="••••••••"
                  value={loginPassword}
                  onChange={(e) => setLoginPassword(e.target.value)}
                  required
                  disabled={isSubmitting}
                  data-testid="login-password"
                />
              </div>
              <button
                type="submit"
                className="ui-button ui-button--primary auth-card__btn"
                disabled={isSubmitting}
                data-testid="login-submit"
              >
                {isSubmitting ? 'Logowanie...' : 'Zaloguj się'}
              </button>
            </form>

            <div className="auth-divider">
              <span>lub</span>
            </div>

            <button
              type="button"
              className="auth-google-btn auth-card__btn"
              onClick={() => void loginWithGoogle()}
              disabled={isSubmitting}
              data-testid="login-google"
            >
              <svg className="auth-google-btn__icon" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              Zaloguj przez Google
            </button>

            <p className="auth-card__footer">
              Nie masz konta?{' '}
              <button
                type="button"
                className="auth-card__link-btn"
                onClick={() => switchTab('register')}
                data-testid="switch-to-register"
              >
                Zarejestruj się
              </button>
            </p>
          </>
        ) : (
          <>
            <form className="auth-card__form" onSubmit={(e) => void handleRegister(e)}>
              <div className="auth-card__form-row">
                <div className="ui-form-group">
                  <label htmlFor="reg-first-name">Imię</label>
                  <input
                    id="reg-first-name"
                    type="text"
                    autoComplete="given-name"
                    placeholder="Jan"
                    value={regFirstName}
                    onChange={(e) => setRegFirstName(e.target.value)}
                    disabled={isSubmitting}
                    data-testid="register-first-name"
                  />
                </div>
                <div className="ui-form-group">
                  <label htmlFor="reg-last-name">Nazwisko</label>
                  <input
                    id="reg-last-name"
                    type="text"
                    autoComplete="family-name"
                    placeholder="Kowalski"
                    value={regLastName}
                    onChange={(e) => setRegLastName(e.target.value)}
                    disabled={isSubmitting}
                    data-testid="register-last-name"
                  />
                </div>
              </div>
              <div className="ui-form-group">
                <label htmlFor="reg-email">Email</label>
                <input
                  id="reg-email"
                  type="email"
                  autoComplete="email"
                  placeholder="jan@firma.pl"
                  value={regEmail}
                  onChange={(e) => setRegEmail(e.target.value)}
                  required
                  disabled={isSubmitting}
                  data-testid="register-email"
                />
              </div>
              <div className="ui-form-group">
                <label htmlFor="reg-password">Hasło</label>
                <input
                  id="reg-password"
                  type="password"
                  autoComplete="new-password"
                  placeholder="Min. 8 znaków"
                  value={regPassword}
                  onChange={(e) => setRegPassword(e.target.value)}
                  required
                  minLength={8}
                  disabled={isSubmitting}
                  data-testid="register-password"
                />
              </div>
              <div className="ui-form-group">
                <label htmlFor="reg-confirm-password">Powtórz hasło</label>
                <input
                  id="reg-confirm-password"
                  type="password"
                  autoComplete="new-password"
                  placeholder="Powtórz hasło"
                  value={regConfirmPassword}
                  onChange={(e) => setRegConfirmPassword(e.target.value)}
                  required
                  minLength={8}
                  disabled={isSubmitting}
                  data-testid="register-confirm-password"
                />
              </div>
              <button
                type="submit"
                className="ui-button ui-button--primary auth-card__btn"
                disabled={isSubmitting}
                data-testid="register-submit"
              >
                {isSubmitting ? 'Tworzenie konta...' : 'Utwórz konto'}
              </button>
            </form>

            <div className="auth-divider">
              <span>lub</span>
            </div>

            <button
              type="button"
              className="auth-google-btn auth-card__btn"
              onClick={() => void loginWithGoogle()}
              disabled={isSubmitting}
              data-testid="register-google"
            >
              <svg className="auth-google-btn__icon" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              Zarejestruj przez Google
            </button>

            <p className="auth-card__footer">
              Masz już konto?{' '}
              <button
                type="button"
                className="auth-card__link-btn"
                onClick={() => switchTab('login')}
                data-testid="switch-to-login"
              >
                Zaloguj się
              </button>
            </p>
          </>
        )}
      </div>
    </main>
  )
}
